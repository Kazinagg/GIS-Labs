...
function FieldExists(Table: TFDTable; FieldName: string): boolean;
...
function CopyScalarFieldsStructure(const Source: TFDTable; var Dest: TFDTable): boolean;
...
function ReadPolygon(TablePolyFace, TablePolyVert: TFDTable;
...
begin
  ...
  Table.Open(NewTableName);
  Query := TFDQuery.Create(nil);
  try
      Query.Sql.Add('SELECT * FROM "' + OldTableName + '"');
      Query.Open;
      try
        Table.FieldDefs.Clear;
        MaxLen := Length(Query.Fields[0].FieldName);
        for I := 1 to Query.FieldCount - 1 do
        begin
          Len := Length(Query.Fields[I].FieldName);
          if Len > MaxLen then
            MaxLen := Len;
        end;
        Table.FieldDefs.Add(Query.Fields[0].FieldName, ftString, MaxLen + 1, False);
        for I := 1 to Query.RecordCount do
        begin
          Query.RecNo := I;
          Table.FieldDefs.Add(Query.Fields[0].AsString, ftFloat, 0, False);
        end;
        Table.CreateTable;
        Table.Open;
        for I := 1 to Query.FieldCount - 1 do
        begin
          Table.Append;
          Table.Fields[0].AsString := Query.Fields[I].FieldName;
          Table.Post;
        end;
        for I := 1 to Query.FieldCount - 1 do
        begin
          ...
          begin
            Query.RecNo := J;
            Table.Fields[J].AsString := Query.Fields[I].AsString;
          end;
          Table.Post;
        end;
      finally
...
var
  ...
  Query: TFDQuery;
begin
  Table := TFDTable.Create(Application);
  try
    Query := TFDQuery.Create(Application);
    try
...
begin
  ...
  dest := TFDTable.Create(Application);
  src  := TFDTable.Create(Application);
...
function isEqualStructure(TableName1, TableName2: TFileName): boolean;
var
  Table1, Table2: TFDTable;
  I: integer;
begin
  Result := False;
  Table1 := TFDTable.Create(Application);
  try
    Table2 := TFDTable.Create(Application);
    try
...
begin
  Result := Length(S) > 0;
  I      := 1;
  Result := Result and (S[1] in ['A'..'Z', 'a'..'z', '_']);
  while Result and (I <= length(s)) do
  begin
    Result := S[i] in ['A'..'Z', 'a'..'z', '0'..'9', '_'];
    Inc(I);
  end;
end;
...
function ReadFramework(ATableName: TFileName; var X0, Y0, Z0: Double;
  var DX, DY, DZ: Double; var NX, NY, NZ: Integer): boolean;
var
  FrameworkTable: TFDTable;
begin
  FrameworkTable := TFDTable.Create(nil);
  try
...
//?    Screen.Cursor := OldCursor;
  ...
  Result := True;
  if ProgressBar <> nil then
    ProgressBar.Position.X := ProgressBar.Min;
end;
