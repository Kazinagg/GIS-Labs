...
C
      ...
      LOGICAL   LOP
      CHARACTER FINDEX*30,FLNAME*200,FLFORM*15,FLSTAT*15,FILACT*20, ! emrl flname 50to200
     &          ACCARG*20
C
      INCLUDE  'FILESPEC.INC'
C
...
C--OPEN FILE
  200 INQUIRE(UNIT=ABS(IN),OPENED=LOP)
      IF(.NOT.LOP) THEN
c emrl        I=INDEX(FLNAME,' ')-1
c emrl        OPEN(ABS(IN),FILE=FLNAME(1:I),ERR=20,
        OPEN(ABS(IN),FILE=FLNAME,ERR=20,
   ...
   20 IF(IDFL.GT.0) THEN
      ...
      ELSE
        ...
        GOTO 10
      ENDIF
    2 FORMAT(1X,'Error: File Cannot Be Found or Opened =>',A200) ! emrl 50to200
    3 FORMAT(1X,'Error: File Cannot Be Found or Opened =>',A200  ! emrl 50to200
     &      /1X,'Please Try Again =>')
C
