...
C     ------------------------------------------------------------------
      call writepestjac(npe,nd,x,ipptr,wt,icut)            !jd
      if(icut.ne.0) then                                   !jd
        write(iout,8000)                                   !jd
8000    format(/,' ',79('-'),/)                            !jd
        write(iout,8010)                                   !jd
8010    format(' SENSITIVITY FILES NOT WRITTEN AS ',       !jd
     +  'PEST INTERFACE PROCESS IS ACTIVE.')               !jd
        write(iout,8000)                                   !jd
        return                                             !jd
      end if                                               !jd
      if(limop.ne.0) return                                !jd
C
...
cjd            WRITE (IUGDO(2),540) WT2*H(N), WT2*HOBS(N), IPLOT(N),
cjd     &                           OBSNAM(N)
cjd            WRITE (IUGDO(3),540) WT2*H(N), WTR, IPLOT(N), OBSNAM(N)
cjd            WRITE (IUGDO(4),550) RES, IPLOT(N), OBSNAM(N)
cjd            WRITE (IUGDO(5),550) WTR, IPLOT(N), OBSNAM(N)
            if(limop.eq.0)
   ...
   10     CONTINUE
     ...
     &                                  HOBS(N), H(N), RES, WT2, WTR
        ...
        IF (IO.EQ.1) THEN
          IF (OUTNAM.NE.'NONE') THEN
            WRITE (IUGDO(1),540) H(N), HOBS(N), IPLOT(N), OBSNAM(N)
cjd            WRITE (IUGDO(2),540) SWH, OWH, IPLOT(N), OBSNAM(N)
cjd            WRITE (IUGDO(3),540) SWH, WTR, IPLOT(N), OBSNAM(N)
cjd            WRITE (IUGDO(4),550) RES, IPLOT(N), OBSNAM(N)
cjd            WRITE (IUGDO(5),550) WTR, IPLOT(N), OBSNAM(N)
            if(limop.eq.0)
