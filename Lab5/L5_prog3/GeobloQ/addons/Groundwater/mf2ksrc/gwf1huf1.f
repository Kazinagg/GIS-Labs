...
C     ------------------------------------------------------------------
      if(minthick.gt.0.0)then
        write(*,7777) trim(aspinfile)
        write(iout,7777) trim(aspinfile)
7777    format(/,' A POSITIVE MINTHICK VALUE HAS BEEN ',
     +  'SUPPLIED IN ASP FILE ',a)
        write(*,7778)
        write(iout,7778)
7778    format(/,' THIS IS NOT ALLOWED WHEN USING THE HUF PACKAGE.')
        stop
      end if
      IREWND=0
      ZERO=0.
