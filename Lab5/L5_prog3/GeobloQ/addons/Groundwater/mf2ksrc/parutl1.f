...
C  specified layer.
         II=IP
         call SetCurrentIndex(ptyp,ilay)                              !jd
...
C  PRINT THE ARRAY.
 200  continue                                                        !jd
      nhow_to_sum = GetSumType()                                      !jd
      if(nhow_to_sum.eq.1)then                                        !jd
        do irow=1,nrow                                                !jd
          do icol=1,ncol                                              !jd
            if(zz(icol,irow).lt.87)zz(icol,irow)=exp(zz(icol,irow))   !jd
          end do                                                      !jd
        end do                                                        !jd
      end if                                                          !jd
      call apply_bounds(ncol,nrow,zz)                                 !jd
      call UnSetCurrentIndex()                                        !jd
cjd  200 CALL ULAPRWC(ZZ,NCOL,NROW,ILAY,IOUT,IPF,ANAME)
         CALL ULAPRWC(ZZ,NCOL,NROW,ILAY,IOUT,IPF,ANAME)
C
      RETURN
      END
...
C                 zone value, modify the array value.
                  IF(IZON(J,I,IZ).EQ.IPCLST(JJ,IC)) THEN
                    ...
                    if(nhow_to_sum.eq.0)then                          !jd
                    ZZ(J,I)=ZZ(J,I)+AA*B(IP)
                    else                                              !jd
                    if(b(ip).le.0.0) go to 8000                       !jd
                    zz(j,i)=zz(j,i)+aa*log(b(ip))                     !jd
                    end if                                            !jd
                    NSUB=NSUB+1
                  END IF
   30           CONTINUE
...
C
      RETURN
8000  write(*,8010)                                                      !jd
8010  format(/,' Error in parameter calculation:-')                      !jd
      write(*,8020)                                                      !jd
8020  format(/,' MODFLOW array generation for at least one ',            !jd
     +'parameter type is by summation of',/,' logs; however at least ',  !jd
     +'one parameter contributing to that array is negative. ',          !jd
     + /,' Hence the array value cannot be calculated.',/)               !jd
       write(*,8030)                                                     !jd
8030   format(/,' Alter information contained in ASP input file.')       !jd
       stop
      END
