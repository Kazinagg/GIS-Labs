...
c-------ECGL JIG
     ...
     &  xk1(12), xk2(12), ang(mxeles), hlast(mxnods), count(mxnods),
     &  nbc(mxnods), ndmp, numnp, numel, np(5,mxeles), lplx
     ...
     &              out(mxnods),outv(mxnods,3)
      common / unsat / uspar(2, 12), iuntyp
      real * 4 out, outv
      character*80 hed
      character*4 lplx
      character*1 flonet
...
c
      ...
      write(22) (out(i),i=1,j)
      write(22) iend
c when ready      write(22) ivec
c when ready      write(22) inode,icount
c when ready      write(22) ielem,numel
c when ready      fname = 'gradient'
c when ready      write(22) iname,fname
c when ready      write(22) its,istate,time
c when ready      j = 0
c when ready      do i=1,numnp
c when ready        if (idnew(i) .ne. -1) then
c when ready          j = j + 1
c when ready          outv(j,1) = gradx(i)
c when ready          outv(j,2) = grady(i) 
c when ready          outv(j,3) = 0.d0 
c when ready        endif
c when ready      enddo
c when ready      write(22) ((outv(i,k),k=1,3),i=1,j)
c when ready      write(22) iend
c
...
c
      if (nod .eq. 4) then
        call grvel4 (xx, yy, hh, thetd, mat, grad, v)
      else
        call grvel3 (xx, yy, hh, thetd, mat, grad, v)
      end if
c
  ...
  130 dir = datan2 (v(2), v(1)) / dr + thetd
      go to 150
  140 dir = 0.d0
  150 write(16,180) n, v(1), v(2), thetd, v3, dir
c
      xc = xc / float (nod)
      yc = yc / float (nod)
      cc = dcos (dir * dr)
...
c
      ...
      include 'seep.inc'
      common fx(mxnods), flow(mxnods), x(mxnods), y(mxnods),
     &  xk1(12), xk2(12), ang(mxeles), frx(mxnods), count(mxnods),
     &  nbc(mxnods), ndmp, numnp, numel, np(5,mxeles), lplx
      common  / banarg / mband, numblk, r(mxnods), c(2*mxbndw,mxbndw),
     &                   nd, nd2
      common / fnet / q, hmin, hmax, smin, smax,
...
c
  550 write(16,560) nn
  560 format(' zero or negative area element', i5 / )
      ndmp = 1
  570 return
      end
c ======================================================================
...
c
  ...
  600   write(16,950) n, head, perc, ibl, iast, ibl, 
     &                x(n), y(n)
        go to 640
  610   write(16,960) n, head, perc, flow(n), ibl, iast, ibl,
     &                x(n), y(n)
  ...
  620   write(16,930) n, head, perc, ibl, ibl, iast
        go to 640
  630   write(16,920) n, head, perc, flow(n), ibl, ibl, iast
        if (flow(n).gt.0.d0) qp = qp + flow(n)
        izero = 0
  ...
  660   write(16,980) n, head, perc, flow(n)
        if (flow(n).gt.0.d0) qp = qp + flow(n)
        izero = 0
        go to 680
...
c ======================================================================
      subroutine grvel3 (xx, yy, hh, angle, mat, grad, v)
c
c
...
c
      det = a12 + a23 + a31
      if (det. eq. 0.d0) then
        print*, 'An element with zero area has been encountered.'
...
c
c ======================================================================
      subroutine grvel4 (xx, yy, hh, angle, mat, grad, v)
c
c
c         This subroutine computes the gradient and discharge velocity
...
c ======================================================================
	...
	common/emrl/stayopen
      ...
      if (stayopen.eq.'1') then
        write(*,*) ' '
        write(*,4000)
4000    format(' Press ENTER to quit.')
        read(*,'(a)') dummyvar
	endif
      stop
