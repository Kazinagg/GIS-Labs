...
C*     Block 6: Definition of full (n x n) analytical jacobian matrix*
	...
	ELSE  !eqns for variable rxn parameters
	 ...
	 pd(2,4) = 0.0d0
         pd(3,1) = 0.0d0
         pd(3,2) = vrc(j,i,k,2)*vrc(j,i,k,6)/reta(3)
	 pd(3,3) = -vrc(j,i,k,3)/reta(3)
	 pd(3,4) = 0.0d0
         pd(4,1) = 0.0d0
         pd(4,2) = 0.0d0
	 pd(4,3) = vrc(j,i,k,3)*vrc(j,i,k,7)/reta(4)
	 pd(4,4) = -vrc(j,i,k,4)/reta(4)
	END IF
C*     End of Block 6*
       RETURN
       END
...
C-------EMRL JIG
        ...
        END
C
C*****Reaction Model#8****************************************************C
c     Biodegradation coupled with NAPL Dissolution Module
c	Module with three species, four constant reaction parameters,
c	and one variable reaction parameter. The species are: pce, tce,
c	pce_napl. The constant rxn parameters are: Pce*, Naplmax (Pcenaplo),
c	Kpce,and Ktce. Variable rxn parameter is: KLa
c     To use this option set ncomp=3,mcomp=2,NCRXNDATA=4 and NVRXNDATA=1
...
c	INTEGER 
      DOUBLE PRECISION KLa,PCEstar,PCEnaplo,Kpce,Ktce,ytcepce,
     &        pce,tce,pcenapl 
C*End of block 3********************************************************
...
c      *Initilize reaction parameters here, if required*
      IF (First_time .EQ. 1) THEN
          ...
          PCEnaplo = rc(2) ! maximum NAPL concentration
          Kpce = rc(3) 	! biodecay coefficient for pce
          Ktce = rc(4) 	! biodecay coefficent for TCE
...
c      *Differential Reaction Equations*
	dydt(1) = (KLa*pcenapl*(pcestar-pce)/
     &          (reta(1)*pcenaplo)) - (Kpce*pce/reta(1))
      dydt(2) = (ytcepce*Kpce*pce/reta(2)) - (Ktce*tce/reta(2))
      dydt(3) = -KLa*pcenapl*poros*(pcestar-pce)/(pcenaplo*rhob)
C*End of block 6********************************************************
...
C*****Reaction Model#9****************************************************C
c     Dual Domain (or Porosity) Module
c	Module with two species, and two constant reaction parameters
c	The species are: mobile_conc, immobile_conc
c	The constant rxn parameters are: eta, teta_im
c	Note :The mobile porosity should be entered as the regular effective
c	porosity in the BTN file
C*************************************************************************C
...
c      *Declare your problem-specific new variables here*
c      INTEGER 
       DOUBLE PRECISION eta,teta_im
C*End of block 3********************************************************

C*Block 4:**************************************************************
c      *Initilize reaction parameters here, if required*
c	rc(1)  = First order mass transfer coefficient "eta" [1/T]
c	rc(2)  = Immobile porosity (teta_im)
        IF (First_time .EQ. 1) THEN
	     eta = rc(1)
           teta_im = rc(2)
           First_time = 0 !reset First_time to skip this block later
...
C*Block 6:**************************************************************
c      *Differential Reaction Equations*
c        y(1)  = Mobile-phase concentration [M/L3]
c        y(2)  = Immobile-phase concentration
         dydt(1) = eta*(y(2)-y(1))/(poros*reta(1))
         dydt(2) = -eta*(y(2)-y(1))/teta_im
C*End of block 6********************************************************
...
c     Example below is identical to rxneqn6
c-------------------------------------------------------	   
	...
	DOUBLE PRECISION rc
        ...
        COMMON /rxnshare/dpporos,dprhob,dpreta(100),nvrxndata,j,i,k
        COMMON /rxnshare1/nlay,nrow,ncol
C        !DEC$ ATTRIBUTES DLLIMPORT :: rxns
	...
	DOUBLE PRECISION rc
        ...
        COMMON /rxnshare/dpporos,dprhob,dpreta(100),nvrxndata,j,i,k
        COMMON /rxnshare1/nlay,nrow,ncol
C        !DEC$ ATTRIBUTES DLLIMPORT :: jrxns
