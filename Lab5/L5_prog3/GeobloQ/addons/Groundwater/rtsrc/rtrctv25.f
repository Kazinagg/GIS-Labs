...
Cphase concentration via the ssm package, but RT3Dv2.5 will not allow this option)
...
C
      ...
      double precision t,tout,y,atol,rtol,work
      COMMON /solver_consts/ atol(100),rtol(100)
     ...
     &            iwork(1))
      IF (iflag .NE. 2) THEN
 	   write (*,*) "Reaction solver failed...." 
         ...
         if (iflag.eq.-2) then
           write (*,*) "Took a single successful step."
         elseif (iflag.eq.3) then
            write (*,*) "Relative error tolerance too small."
         elseif (iflag.eq.4) then
            write (*,*) "More than 3000 derivative evaluations "
     &              // "required."
         elseif (iflag.eq.5) then
            write (*,*) "Solution vanished; cannot do pure relative "
     &                 // "error test.  Make atol non-zero."
         elseif (iflag.eq.6) then
            write (*,*) "Requested accuracy could not be achieved.  "
     &                // "Increase error tolerance(s)."
         elseif (iflag.eq.7) then
            write (*,*) "Runge-Kutta-Fehlberg is too inefficient to "
     &                // "use on this problem."
