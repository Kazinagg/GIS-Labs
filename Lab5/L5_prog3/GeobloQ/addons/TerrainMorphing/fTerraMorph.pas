...
type
  TFormTerramorph = class(TForm)
    ...
    procedure ExplpxCombine(F: TColor32; var B: TColor32; M: TColor32); // GR32
    procedure ExplpixCombine(F: TColor; var B: TColor; M: TColor);
...
var
  FormTerramorph: TFormTerramorph;
  mx, my: Integer;
  angx, angy: Single;
  cx, cy: Integer;
  px, py: Integer;
  pressedl, pressedr, pressedm: Boolean;
  bufang, campos: TGLVector;
...
procedure UVPlanarMapping(Vector: TVector3f; min, max: TAffineVector; var U, V: Single);
var
...
procedure TFormTerramorph.FormCreate(Sender: TObject);
begin
...
///  hole.OnPixelCombine := explpxcombine;
  ...
  GLMaterialLibrary1.LibMaterialByName('Terrain').Material.
     Texture.Image.GetBitmap32.Assign(terrtex);

  FMin := GLTerrainRenderer1.Position.AsAffineVector;
  FMax := AffineVectorMake(cMapSize * GLTerrainRenderer1.scale.X, 0, cMapSize * GLTerrainRenderer1.scale.Y);
...
// GR32
procedure TFormTerramorph.explpxcombine(F: TColor32; var B: TColor32; M: TColor32);
begin
  B := ColorModulate(F, B);
end;

procedure TFormTerramorph.Explpixcombine(F: TColor; var B: TColor; M: TColor);
begin
  B := ColorModulate(F, B);
end;


procedure TFormTerramorph.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #27 then
    close;
end;

procedure TFormTerramorph.CheckCameraControls(deltaTime, newTime: Double);
var
  bufv: TGLVector;
  ps: TPoint;
  curspeed: Single;
...
procedure TFormTerramorph.GLCadencer1Progress(Sender: TObject; const deltaTime, newTime: Double);
begin
...
procedure TFormTerramorph.AsyncTimer1Timer(Sender: TObject);
begin
  Caption := FloatToStr(GLSceneViewer1.FramesPerSecond);
  GLSceneViewer1.ResetPerformanceMonitor;
end;

procedure TFormTerramorph.GLSceneViewer1MouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFormTerramorph.GLSceneViewer1MouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
...
begin
  ...
  begin
    ...
    SetVector(rayVector, GLSceneViewer1.Buffer.ScreenToVector(affinevectormake(px, GLSceneViewer1.height - py, 0)));
    ...
    begin
      ...
      explosion.DrawTo(terrain, rect(terx - 50, tery - 50, terx + 50, tery + 50));

      hole.Rotate90;
      hole.DrawTo(terrtex, rect(texx - 100, texy - 100, texx + 100, texy + 100));

      terrain.DrawTo(GLBitmapHDS1.Picture.Bitmap.canvas.handle, 0, 0);

      GLMaterialLibrary1.LibMaterialByName('Terrain').Material.Texture.Image.GetBitmap32.Assign(terrtex);
      GLMaterialLibrary1.LibMaterialByName('Terrain').Material.Texture.Image.NotifyChange(self);
      GLBitmapHDS1.MarkDirty(rect(terx - 50, tery - 50, terx + 50, tery + 50));
    end;
  end;
end;

procedure TFormTerramorph.GLSceneViewer1MouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
begin
  cx := X;
  cy := Y;
end;

procedure TFormTerramorph.FormMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint;
  var Handled: Boolean);
var
  curspeed: Single;
begin
...
procedure TFormTerramorph.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  AsyncTimer1.enabled := false;
end;

procedure TFormTerramorph.GLTerrainRenderer1GetTerrainBounds(var l, t, r, B: Single);
begin
  l := 0;
  t := cMapSize;
  r := cMapSize;
