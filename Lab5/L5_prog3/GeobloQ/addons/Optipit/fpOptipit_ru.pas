...
type
  ...
  private
    ...
    tl: // поля в строке
    TStringList;
    NumLine: Integer; // номер строки
    IDBlock: Integer; // идентификатор блока

    Btype: Integer; // тип породы в блоке
...
(*sub*)function ReadBlocks: Boolean;
  ...
  begin
    ...
    begin
      tl.CommaText := sl[i];
      IDBlock := StrToInt(tl[0]); // читаем Id блока
      X := StrToInt(tl[1]);
      Z := StrToInt(tl[2]);
      Y := StrToInt(tl[3]);
...
begin
  ...
  OpenTextFileDialog1.InitialDir := DataDir;
  ...
  try
    ...
    case OpenTextFileDialog1.FilterIndex of
      1:
        ReadBlocks; // считывает координаты и показатели блоков
      2:
        ReadPrecedences;
      3:
        ReadUltimatePit;
