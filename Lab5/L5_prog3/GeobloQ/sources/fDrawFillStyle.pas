...
type
  TFormDrawFillStyle = class(TFormBlankDialog)
    ToolBar:      TToolBar;
...
var
  FormDrawFillStyle: TFormDrawFillStyle;
...
procedure TFormDrawFillStyle.FormCreate(Sender: TObject);
begin
  inherited;
  ReadIniFile;
...
constructor TFormDrawFillStyle.CreateServicePolygons(AOwner: TComponent;
  TheFillStyle: TFillStyle; IsLine: boolean);
begin
...
procedure TFormDrawFillStyle.DrawExample;
var
  vPen: TPen;
  BMap: TBitmap;
  BrushStyle: TBrushStyle;
...
procedure TFormDrawFillStyle.UpdateListPatterns;
var
  I:    longint;
...
procedure TFormDrawFillStyle.UpdateListTextures;
var
  I:    longint;
  BMapPars: TBitMapPars;
  Bmap: TBitmap;
...
procedure TFormDrawFillStyle.ButtonLineStyleClick(Sender: TObject);
begin
  FormDrawLineStyle := TFormDrawLineStyle.CreateServiceLines(
    FillStyle.PenWidth, FillStyle.PenColor, FillStyle.PenStyle);
  if FormDrawLineStyle.ShowModal = mrOk then
  begin
    FillStyle.PenWidth := FormDrawLineStyle.LineOptions.Width;
    FillStyle.PenColor := FormDrawLineStyle.LineOptions.Color;
    FillStyle.PenStyle := FormDrawLineStyle.LineOptions.Style;
  end;
  FormDrawLineStyle.Free;
  DrawExample;
end;

procedure TFormDrawFillStyle.ToolButtonNewClick(Sender: TObject);
var
  Picture: TPicture;
begin
...
procedure TFormDrawFillStyle.ToolButtonDeleteClick(Sender: TObject);
begin
  case PageControl.ActivePageIndex of
    0:
    begin
...
procedure TFormDrawFillStyle.ToolButtonEditClick(Sender: TObject);
var
...
procedure TFormDrawFillStyle.ListBoxDrawItems(Control: TWinControl;
...
procedure TFormDrawFillStyle.ImageFillColorClick(Sender: TObject);
begin
...
procedure TFormDrawFillStyle.ImageBkColorClick(Sender: TObject);
begin
...
procedure TFormDrawFillStyle.PanelBkColorEnter(Sender: TObject);
begin
  with ImageBkColor.Canvas do
  begin
...
procedure TFormDrawFillStyle.PanelFillColorEnter(Sender: TObject);
begin
  with ImageFillColor.Canvas do
  begin
...
procedure TFormDrawFillStyle.CheckBoxBackgroundClick(Sender: TObject);
begin
...
procedure TFormDrawFillStyle.FormClose(Sender: TObject; var Action: TCloseAction);
var
  I: integer;
begin
  if ListBoxTexture.Items.Count <> 0 then
...
procedure TFormDrawFillStyle.FormDestroy(Sender: TObject);
begin
  FLevel.Free;
  TableTexture.Close;
  TablePattern.Close;
...
procedure TFormDrawFillStyle.SetLevel(const Value: TLevel);
begin
  Level.Assign(Value);
end;

function TFormDrawFillStyle.GetLevel: TLevel;
begin
  if FLevel = nil then
  begin
...
procedure TFormDrawFillStyle.ReadIniFile;
begin
...
procedure TFormDrawFillStyle.WriteIniFile;
begin
  IniFile := TIniFile.Create(ChangeFileExt(ParamStr(0), '.ini'));
  with IniFile do
  begin
