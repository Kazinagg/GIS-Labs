...
type
  TfrmMapWindow = class(TFormFirst)
    TableMap:     TTable;
...
procedure TfrmMapWindow.OpenNewModel(AFileName: string; AModelType: integer);
var
  OldCursor: TCursor;
begin
  ...
  try
    ...
    try
      if ModelIndex >= 0 then
        FormGeoblock.EnableMapItems(Model.ModelType, False);   //Enables menu commands
    except
    end;
    case AModelType of
    ...
    try
      ...
      try
        ...
        frmViewProjectManager.AddToManager;
      except
        on E: Exception do
        begin
  ...
  try
    FormGeoblock.EnableMapItems(AModelType, True);
    FormGeoblock.EnableFileItems(True);
  except
  end;
  ViewZoomToAll;
end;

procedure TfrmMapWindow.InitMapWin(AFileName: string; AModelType: integer);
begin
  try
...
//===================== private =========================
procedure TfrmMapWindow.DefineCoords;
var
  RatioF: Single;
begin
  with GBCanvas do
...
//====================== public ======================
procedure TfrmMapWindow.BackColorDlg;
begin
  with dmDialogs do
  begin
...
//======================== Load Datasets =======================
procedure TfrmMapWindow.RemoveModel(AModelID: integer);
var
  AttributeNo: integer;
  Temp: TGBModel;
begin
...
procedure TfrmMapWindow.ChangeModel(AModelID: integer);
var
  AttribNo: integer;
begin
  ...
  FormGeoblock.EnableMapItems(ModelList[ModelIndex].ModelType, False);
  ModelIndex := AModelID;
  FormGeoblock.EnableMapItems(ModelList[AModelID].ModelType, True);
...
procedure TfrmMapWindow.PerformCrossSection;
begin
  {...}
end;

procedure TfrmMapWindow.LightDlg;
begin
...
procedure TfrmMapWindow.ViewScale;
begin
...
procedure TfrmMapWindow.ViewTop;
begin
  GBCanvas.ViewTop;
end;

procedure TfrmMapWindow.ViewBottom;
begin
  GBCanvas.ViewBottom;
end;

procedure TfrmMapWindow.ViewFront;
begin
  GBCanvas.ViewFront;
end;

procedure TfrmMapWindow.ViewBack;
begin
  GBCanvas.ViewBack;
end;

procedure TfrmMapWindow.ViewRight;
begin
  GBCanvas.ViewRight;
end;

procedure TfrmMapWindow.ViewLeft;
begin
  GBCanvas.ViewLeft;
end;

procedure TfrmMapWindow.View3D;
begin
  GBCanvas.View3D;
end;

procedure TfrmMapWindow.ViewRotate;
begin
...
procedure TfrmMapWindow.ViewDefault;
begin
  with GBCanvas do
  begin
    if FormGeoblock.Observe3D.Checked then
    begin
      wAngleY := DefwAngleY;
      wAngleX := DefwAngleX;
      wAngleZ := DefwAngleZ;
...
procedure TfrmMapWindow.Zoom;
begin
  GBCanvas.ZoomIn;
end;

procedure TfrmMapWindow.ViewXYZ;
begin
  with GBCanvas do
  begin
    View3D;
...
procedure TfrmMapWindow.ViewXY;
begin
  with GBCanvas do
  begin
    ViewTop;
...
procedure TfrmMapWindow.ViewXZ;
begin
  with GBCanvas do
  begin
    GBCanvas.ViewFront;
...
procedure TfrmMapWindow.ViewYZ;
begin
  with GBCanvas do
  begin
    GBCanvas.ViewLeft;
...
procedure TfrmMapWindow.FormKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
var
  K: Float;
begin
  ...
  begin
    case Key of
      ...
      VK_SUBTRACT:
        ...
        else if FormGeoblock.ViewVolumeXYZ.Checked then
        begin // rotate
          case Key of
            VK_LEFT:
            begin
...
procedure TfrmMapWindow.SaveAs(FileName: TFileName);
var
  Image:     TBitmap;
...
procedure TfrmMapWindow.ViewZoomToAll;
var
  I: integer;
...
procedure TfrmMapWindow.ViewZoomToModel;
begin
...
procedure TfrmMapWindow.GBCanvasMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: integer);
var
...
begin
  ...
  begin
    ...
    if ssLeft in Shift then
      if FormGeoblock.ViewSelect.Checked then
        Exclude(NewShift, ssLeft)
      else
      if FormGeoblock.ViewScroll.Checked then
        Include(NewShift, ssAlt)
      else
      if FormGeoblock.ViewPan.Checked then
        Include(NewShift, ssLeft)
      else
      if FormGeoblock.aZoomInOut.Checked then
      begin
...
procedure TfrmMapWindow.GBCanvasMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: integer);
var
  MouseDown: TPoint;
begin
...
procedure TfrmMapWindow.GBCanvasDblClick(Sender: TObject);
begin
  GBCanvas.DefaultMouseDblClick(Sender);
end;

procedure TfrmMapWindow.MenuItemMapSavePolygonClick(Sender: TObject);
var
  TablePolygon: TTable;
  TableVertex: TTable;
  PolyID: integer;
...
//----------------------------------------------------------------
procedure TfrmMapWindow.ExportToMIF(AFileName: TFileName);
var
...
procedure TfrmMapWindow.DoExportToMIF;
begin
...
procedure TfrmMapWindow.SetModel(const Value: TGBModel);
begin
  Model.Assign(Value);
end;

function TfrmMapWindow.GetModel: TGBModel;
begin
  Result := ModelList[ModelIndex];
end;

function TfrmMapWindow.GetModelCount: integer;
begin
  Result := ModelList.Count;
end;

procedure TfrmMapWindow.SetModelList(const Value: TModelList);
begin
  FModelList := Value;
end;

function TfrmMapWindow.GetModelList: TModelList;
begin
...
procedure TfrmMapWindow.SetModelIndex(const Value: integer);
begin
...
function TfrmMapWindow.GetModelIndex: integer;
begin
  Result := Min(ModelCount - 1, Max(0, FModelIndex));
end;

procedure TfrmMapWindow.ReloadModel;
begin
  if ModelCount > 0 then
  begin
    Model.Close;
...
procedure TfrmMapWindow.GBCanvasCallLists(Sender: TObject);
var
  I: integer;
  V: TVertex;
...
procedure TfrmMapWindow.ReadIniFile;
begin
...
procedure TfrmMapWindow.WriteIniFile;
begin
  IniFile := TIniFile.Create(ChangeFileExt(ParamStr(0), '.ini'));
  with IniFile do
    try
...
procedure TfrmMapWindow.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  WriteIniFile; //before fmMapWindow := nil!
  try
    TableMap.Close;
    if FormGeoblock.MDIChildCount = 1 then
    begin
      FormGeoblock.EnableFileItems(False);
      FormGeoblock.EnableMapItems(Model.ModelType, False);
    end;
    frmViewProjectManager.Clear;
    frmMapWindow := nil;
    Action      := caFree;
    FModelList.Free;
    FModelList := nil;
...
procedure TfrmMapWindow.Grid2DSelectCell;
begin
  GBCanvas.Cursor := crSelectCursor;
end;

procedure TfrmMapWindow.Grid2DSelectCol;
begin
  GBCanvas.Cursor := crSelectCursor;
end;

procedure TfrmMapWindow.Grid2DSelectRow;
begin
  GBCanvas.Cursor := crSelectCursor;
end;

procedure TfrmMapWindow.GBCanvasSelectPrimitive(Sender: TObject;
