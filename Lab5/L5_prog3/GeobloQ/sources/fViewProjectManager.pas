...
type
  TfrmViewProjectManager = class(TFormBlankDialog)
...
var
  frmViewProjectManager : TfrmViewProjectManager;
...
procedure TfrmViewProjectManager.SpeedButtonAddClick(Sender: TObject);
begin
  FormGeoblock.FileOpenModel.Execute;
end;

procedure TfrmViewProjectManager.SpeedButtonOptionsClick(Sender: TObject);
begin
  FormGeoblock.MapOptions.Execute;
end;


procedure TfrmViewProjectManager.FormActivate(Sender: TObject);
begin
  inherited;
...
procedure TfrmViewProjectManager.lwModelsSelectItem(Sender: TObject;
  Item: TListItem; Selected: boolean);
begin
  if Selected then
    if frmMapWindow <> nil then
      frmMapWindow.ChangeModel(Item.Index);
end;

procedure TfrmViewProjectManager.lwModelsMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: integer);
var
  ListItem: TListItem;
begin
  ListItem := lwModels.GetItemAt(X, Y);
  if (Assigned(frmMapWindow)) and (Assigned(ListItem)) then
    lwModels.Hint := frmMapWindow.ModelList[ListItem.Index].ModelName;
end;

procedure TfrmViewProjectManager.FormPaint(Sender: TObject);
begin
  FRemoved := False;
end;

procedure TfrmViewProjectManager.AddToManager;
...
procedure TfrmViewProjectManager.ToolButtonRemoveClick(Sender: TObject);
var
  Num: integer;
begin
  ...
  begin
    ...
    begin
      ...
      FormGeoblock.EnableMapItems(mtAll, False);
    end;
  end;
  if frmMapWindow <> nil then
...
procedure TfrmViewProjectManager.ToolButtonOpenClick(Sender: TObject);
begin
  FormGeoblock.FileOpenModel.Execute;
end;


procedure TfrmViewProjectManager.CheckListBoxVisibleClick(Sender: TObject);
var
  I: integer;
  Count: Integer;
begin
...
procedure TfrmViewProjectManager.Clear;
begin
...
procedure TfrmViewProjectManager.EnableButtons;
begin
...
procedure TfrmViewProjectManager.LoadFromFile(const FileName: TFileName);
var
  I: integer;
  ...
  try
    ...
    begin
      ...
      case LineType of
        ...
        try
          FormGeoblock.ShowMap(Trim(CurLine), ConvertPLTtoDataType(LineType));
        except
        end;
      end;
    end;
...
//----------------------------------------------------------------------------
procedure TfrmViewProjectManager.SaveToFile(const FileName: TFileName);
...
begin
  ...
  try
    ...
    Writeln(FileStream, LineTypeStrings[pltComment] + ' Written by : Geoblock Team');
    Writeln(FileStream, LineTypeStrings[pltComment] + ' Date       : ' +
      DateTimeToStr(Now));
    Writeln(FileStream);
    if frmMapWindow <> nil then
    begin
      with frmMapWindow, ModelList do
        for I := 0 to Count - 1 do
        begin
...
procedure TfrmViewProjectManager.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caHide;
  inherited;
end;

procedure TfrmViewProjectManager.ButtonOKClick(Sender: TObject);
begin
  FormGeoblock.Repaint;
  Hide;
  inherited;
end;
