...
type
  TFormPerformIsosurfaces = class(TFormBlankDialog)
...
var
  FormPerformIsosurfaces: TFormPerformIsosurfaces;
...
procedure TFormPerformIsosurfaces.ReadIniFile;
begin
...
procedure TFormPerformIsosurfaces.SpinEditClassesChange(Sender: TObject);
begin
  StringGridClasses.RowCount := SpinEditClasses.Value;
end;

procedure TFormPerformIsosurfaces.WriteIniFile;
begin
  IniFile := TIniFile.Create(ChangeFileExt(ParamStr(0), '.ini'));
  with IniFile do
    try
...
procedure TFormPerformIsosurfaces.FormCreate(Sender: TObject);
begin
  inherited;
  ReadIniFile;
end;

procedure TFormPerformIsosurfaces.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  WriteIniFile;
  inherited;
end;

procedure TFormPerformIsosurfaces.CheckBoxPlaneXClick(Sender: TObject);
begin
