...
type
  TFormToolsUnitsConverter = class(TFormPageDialog)
...
var
  FormToolsUnitsConverter: TFormToolsUnitsConverter;
...
procedure TFormToolsUnitsConverter.FormCreate(Sender: TObject);
begin
  inherited;
  SetWorkField;
...
procedure TFormToolsUnitsConverter.FormActivate(Sender: TObject);
begin
  PageControlChange(Self);
end;


procedure TFormToolsUnitsConverter.BeginUpdate;
begin
  Inc(FUpdateCount);
end;

procedure TFormToolsUnitsConverter.EndUpdate;
begin
  FUpdateCount := Max(0, FUpdateCount - 1);
end;

function TFormToolsUnitsConverter.GetInUpdate: boolean;
begin
  Result := boolean(FUpdateCount);
end;

procedure TFormToolsUnitsConverter.SetInUpdate(Value: boolean);
begin
  case Value of
    False: EndUpdate;
    True: BeginUpdate;
...
procedure TFormToolsUnitsConverter.SetWorkField;
begin
  DBGridUnits.Columns.Add;
  DBGridUnits.Columns[0].FieldName := fldNAME;
  DBGridUnits.Columns[0].Title.Caption := 'Parameter'; //LoadResString(@sgbParameter);
  DBGridUnits.Columns[0].Width    := 200;
  DBGridUnits.Columns[0].ReadOnly := True;

  DBGridUnits.Columns.Add;
  DBGridUnits.Columns[1].FieldName := fldVALUE;
  DBGridUnits.Columns[1].Title.Caption := 'Value';//LoadResString(@sgbValue);
  DBGridUnits.Columns[1].Width := 200;

  DBGridUnits.Columns.Add;
  DBGridUnits.Columns[2].FieldName := fldFACTOR;
  DBGridUnits.Columns[2].Title.Caption := 'Factor';//LoadResString(@sgbFactor);
  DBGridUnits.Columns[2].Visible   := False;
end;

procedure TFormToolsUnitsConverter.PageControlChange(Sender: TObject);
begin
...
procedure TFormToolsUnitsConverter.FormDestroy(Sender: TObject);
begin
  TableUnits.Close;
  inherited;
end;

procedure TFormToolsUnitsConverter.TableUnitsAfterPost(DataSet: TDataSet);
var
  Value: double;
  Short: string;
  RecNo: integer;
...
procedure TFormToolsUnitsConverter.DBGridUnitsKeyPress(Sender: TObject; var Key: char);
begin
