...
type
  TfmInterKriging = class(TFormBlankDialog)
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            while not EOF do
            begin
              q := AddChild(t, 'Structure' + ' ' + IntToStr(j));
              ...
              AddChild(q, 'Model function' + ': ' + mf);
              AddChild(q, 'Nugget' + ': ' + FloatToStr(FieldByName(fldNugget).Value));
              AddChild(q, 'Contribution' + ': ' + FloatToStr(
                FieldByName(fldContribution).Value));
              AddChild(q, 'Range' + ': ' + FloatToStr(FieldByName(fldRange).Value));
              AddChild(q, 'Anis1' + ': ' + FloatToStr(FieldByName(fldAnis1).Value));
              AddChild(q, 'Anis2' + ': ' + FloatToStr(FieldByName(fldAnis2).Value));
              AddChild(q, 'Azimuth' + ': ' + FloatToStr(
                FieldByName(fldAzimuthAngle).Value));
              AddChild(q, 'Dip' + ': ' + FloatToStr(FieldByName(fldDipAngle).Value));
              AddChild(q, 'Plunge' + ': ' + FloatToStr(
...
begin
  ...
  if CurrentID <> -1 then
    DBChart.Title.Text.Add(CurrentTable + ' - ' + 'Variogram' +
      ' ' + IntToStr(CurrentID))
  else
    DBChart.Title.Text.Add('No data');
end;
