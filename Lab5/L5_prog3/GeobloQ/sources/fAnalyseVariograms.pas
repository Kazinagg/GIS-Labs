...
type
  TFormAnalyseVariograms = class(TFormMethodDialog)
...
var
  FormAnalyseVariograms: TFormAnalyseVariograms;
...
procedure TFormAnalyseVariograms.SetDirectionData(p: PVariogramDirection);
begin
  with p^ do
  begin
    azimuth := 0;
...
procedure TFormAnalyseVariograms.FormCreate(Sender: TObject);
var
  i: integer;
...
begin
  ...
  begin
    New(p);
    SetDirectionData(p);
    ListBoxDirections.AddItem('Direction' + ' ' + IntToStr(i + 1), TObject(p));
  end;
...
procedure TFormAnalyseVariograms.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  WriteIniFile;
  inherited;
end;

procedure TFormAnalyseVariograms.ButtonResetClick(Sender: TObject);
var
  i: integer;
  p: PVariogramDirection;
begin
...
procedure TFormAnalyseVariograms.DirectionPropertyChange(Sender: TObject);
var
  ed: TEdit;
  i:  integer;
  p:  PVariogramDirection;
...
procedure TFormAnalyseVariograms.ButtonOKClick(Sender: TObject);
begin
  MakeVariogram;
end;

procedure TFormAnalyseVariograms.ListBoxDirectionsClick(Sender: TObject);
var
  i: integer;
...
procedure TFormAnalyseVariograms.ListBoxInputNamesClick(Sender: TObject);
var
...
procedure TFormAnalyseVariograms.ToolButtonInputClick(Sender: TObject);
begin
  inherited;
...
procedure TFormAnalyseVariograms.SpinEditDirectionsNumberChange(Sender: TObject);
var
  DirCount, i: integer;
  p: PVariogramDirection;
begin
...
procedure TFormAnalyseVariograms.ReadIniFile;
begin
...
procedure TFormAnalyseVariograms.WriteIniFile;
begin
  IniFile := TIniFile.Create(ChangeFileExt(ParamStr(0), '.ini'));
  with IniFile do
    try
...
procedure TFormAnalyseVariograms.MakeVariogram;
var
