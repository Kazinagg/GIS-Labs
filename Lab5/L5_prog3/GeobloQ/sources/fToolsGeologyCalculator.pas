...
type
  TFormToolsGeologyCalculator = class(TFormPageDialog)
...
var
  FormToolsGeologyCalculator: TFormToolsGeologyCalculator;
...
procedure TFormToolsGeologyCalculator.FormCreate(Sender: TObject);
begin
  inherited;
  ReadIniFile;
...
procedure TFormToolsGeologyCalculator.PageControlChange(Sender: TObject);
begin
...
procedure TFormToolsGeologyCalculator.BeginUpdate;
begin
  Inc(FUpdateCount);
end;

procedure TFormToolsGeologyCalculator.EndUpdate;
begin
  FUpdateCount := Max(0, FUpdateCount - 1);
end;

function TFormToolsGeologyCalculator.GetInUpdate: boolean;
begin
  Result := boolean(FUpdateCount);
end;

procedure TFormToolsGeologyCalculator.SetInUpdate(Value: boolean);
begin
  case Value of
    False: EndUpdate;
    True: BeginUpdate;
...
//----------------------- Object Height ----------------------------\\
procedure TFormToolsGeologyCalculator.EditScaleObjectHeightChange(Sender: TObject);
...
//----------------------- DThick page ------------------------------\\
procedure TFormToolsGeologyCalculator.EditScaleDoubleVertThicknessChange(
  Sender: TObject);
begin
  if not inUpdate then
  begin
...
procedure TFormToolsGeologyCalculator.EditScaleDoubleHorizThicknessChange(
  Sender: TObject);
begin
  if not inUpdate then
  begin
...
procedure TFormToolsGeologyCalculator.EditScaleDoubleSeamAngleChange(Sender: TObject);
begin
  if not inUpdate then
  begin
...
procedure TFormToolsGeologyCalculator.EditScaleDoubleTrueThicknessChange(
  Sender: TObject);
begin
  if not inUpdate then
  begin
...
procedure TFormToolsGeologyCalculator.EditValueLineamentChange(Sender: TObject);
begin
...
procedure TFormToolsGeologyCalculator.EditValueSeamChange(Sender: TObject);
begin
  { TODO 1 -oPW -cSurvey : Calculate azimuth, dip, strike and area }
end;

procedure TFormToolsGeologyCalculator.FormClose(Sender: TObject;
