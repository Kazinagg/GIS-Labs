...
type
  TFormMethodTriangulation = class(TFormMethodDialog)
...
var
  FormMethodTriangulation: TFormMethodTriangulation;
...
procedure TFormMethodTriangulation.FormCreate(Sender: TObject);
begin
  inherited;
  ReadIniFile;
end;
...
procedure TFormMethodTriangulation.ToolButtonInputClick(Sender: TObject);
begin
  inherited;
  if ToolButtonPoints2D.Down then
  begin
    Caption := rsTriangulation;
    InModelType := mtPoints2D;
    OutModelType := mtTins;
    RadioGroupMethod.Items[0] := 'Delaunay triangulation';
    RadioGroupMethod.Items[1] := 'Restricted triangulation';
    CheckBoxVoronoiDiagram.Caption := 'Voronoi polygons';
    PanelOutPath.Caption := ExpandPath(DirTinFaces);
    PanelOutPath.Hint    := PanelOutPath.Caption;
  end;
  if ToolButtonPoints3D.Down then
  begin
    Caption := rsTetrahedralization;
    ...
    RadioGroupMethod.Items[0] := 'Delaunay tetrahedralization';
    RadioGroupMethod.Items[1] := 'Restricted tetrahedralization';
    CheckBoxVoronoiDiagram.Caption := 'Voronoi polyhedrons';
...
procedure TFormMethodTriangulation.ListBoxInputNamesClick(Sender: TObject);
begin
  inherited;
  RadioGroupMethodClick(Self);
end;

procedure TFormMethodTriangulation.RadioGroupMethodClick(Sender: TObject);
var
  Index: integer;
begin
...
procedure TFormMethodTriangulation.Test;
{
var
TesTT: TTable;
i, nc: integer;
...
procedure TFormMethodTriangulation.ListBoxRealAttributeClick(Sender: TObject);
begin
  inherited;
...
procedure TFormMethodTriangulation.Delaunay2DExecute;
var
  I, J:     integer;
  TIN: TTriangulator2D;
...
====================================================}
procedure TFormMethodTriangulation.ConstrainedTINExecute;
var
...
procedure TFormMethodTriangulation.ReadIniFile;
begin
...
procedure TFormMethodTriangulation.WriteIniFile;
begin
  IniFile := TIniFile.Create(ChangeFileExt(ParamStr(0), '.ini'));
  with IniFile do
    try
...
procedure TFormMethodTriangulation.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  WriteIniFile;
  inherited;
end;

procedure TFormMethodTriangulation.ButtonOKClick(Sender: TObject);
begin
  //inherited;
