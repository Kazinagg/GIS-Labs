...
type
  TFormFileNew = class(TFormPageDialog)
    tsLines: TTabSheet;
    tsAreas: TTabSheet;
...
procedure TFormFileNew.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  WriteIniFile;
  inherited;
end;

procedure TFormFileNew.FormCreate(Sender: TObject);
begin
  inherited;
  ReadIniFile;
  SetSortClasses;
...
procedure TFormFileNew.PageControlChanging(Sender: TObject; var AllowChange: boolean);
begin
...
procedure TFormFileNew.GenerateAndWritePoints(const FileName: TFileName);
var
  I, J: Integer;
  TablePoints : TTable;
...
function TFormFileNew.CreateNewModelTable: boolean;
begin
  Result := True;
  with dmBase do
  begin
...
procedure TFormFileNew.ToolButtonShowAsClick(Sender: TObject);
begin
  ToolBarShowAs.Tag := (Sender as TToolButton).Tag;
end;

procedure TFormFileNew.PageControlChange(Sender: TObject);
begin
...
procedure TFormFileNew.rgLinesClick(Sender: TObject);
begin
...
procedure TFormFileNew.rgAreasClick(Sender: TObject);
begin
...
procedure TFormFileNew.rgSurfacesClick(Sender: TObject);
begin
...
procedure TFormFileNew.rgVolumesClick(Sender: TObject);
begin
  PanelOutPath.Caption := DataBasePath + DirPoints3D;
  case rgVolumes.ItemIndex of
     0: EditOutputName.Text   := 'Box';
...
procedure TFormFileNew.rgDistributionClick(Sender: TObject);
begin
  case rgDistribution.ItemIndex of
    0, 1, 2, 3, 4:
       begin
...
procedure TFormFileNew.SetSortClasses;
var
  I, Sort: integer;
  Lo, Hi:  double;
...
procedure TFormFileNew.SpinEditClassesChange(Sender: TObject);
begin
  inherited;
...
procedure TFormFileNew.WriteIniFile;
begin
...
procedure TFormFileNew.ReadIniFile;
begin
  IniFile := TIniFile.Create(ChangeFileExt(ParamStr(0), '.ini'));
  with IniFile do
    try
...
procedure TFormFileNew.ButtonFunctionClick(Sender: TObject);
begin
  /// Defines a function to place points
end;

procedure TFormFileNew.ButtonOKClick(Sender: TObject);
begin
  inherited;
