...
type
  TFormMethodVarioModeller = class(TFormBlankDialog)
    LabelExpVar: TLabel;
...
var
  FormMethodVarioModeller: TFormMethodVarioModeller;
...
procedure TFormMethodVarioModeller.ReadIniFile;
begin
...
procedure TFormMethodVarioModeller.WriteIniFile;
begin
  IniFile := TIniFile.Create(ChangeFileExt(ParamStr(0), '.ini'));
  with IniFile do
    try
...
procedure TFormMethodVarioModeller.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  WriteIniFile;
  inherited;
end;

procedure TFormMethodVarioModeller.FormCreate(Sender: TObject);
begin
  inherited;
  ReadIniFile;
...
function TFormMethodVarioModeller.GoalFunction(m: TVariogramModel): double;
var
  a, b, i: integer;
  x, y, s: double;
begin
...
procedure TFormMethodVarioModeller.ButtonAutoFitClick(Sender: TObject);
const
  delta = 0.0001;
  alpha = 0.01;
var
...
procedure TFormMethodVarioModeller.ButtonNewVariogramClick(Sender: TObject);
var
  dm: integer;
begin
...
procedure TFormMethodVarioModeller.ButtonOKClick(Sender: TObject);
var
  tblname: string;
  TableModel: TTable;
  i: integer;
...
procedure TFormMethodVarioModeller.UpdateTree;
var
  sr:     TSearchRec;
  FileAttrs: integer;
...
procedure TFormMethodVarioModeller.SetModelData(p: PVariogramModelStructure);
begin
  with p^ do
  begin
...
procedure TFormMethodVarioModeller.SelectData(tblname: string; vid: integer);
begin
  if tblname <> '' then
  begin
...
procedure TFormMethodVarioModeller.CalcModel;
var
  i, a, b: integer;
  m: TVariogramModel;
  x: double;
...
procedure TFormMethodVarioModeller.ModelPropertyKeyDown(Sender: TObject;
...
procedure TFormMethodVarioModeller.ModelPropertyChange(Sender: TObject);
var
  ed: TEdit;
  i:  integer;
  p:  PVariogramModelStructure;
...
procedure TFormMethodVarioModeller.SpinEditNestedStructuresNumberChange(Sender: TObject);
var
  NstCount, i: integer;
  p: PVariogramModelStructure;
begin
  ...
  begin
    if NstCount > NstCountOld then
      ...
      begin
        New(p);
        SetModelData(p);
        AddItem('Structure' + ' ' + IntToStr(i + 1), TObject(p));
      end
...
procedure TFormMethodVarioModeller.TreeViewVariogramClick(Sender: TObject);
var
  id: integer;
  tblName: string;
  p: PVariogramParams;
begin
  ...
  with TreeViewVariogram do
    if Selected <> nil then
      ...
      DBChart.Series[0].Title := 'No data';
  ;
...
procedure TFormMethodVarioModeller.ListBoxNestedStructuresClick(Sender: TObject);
var
  I: integer;
