...
type
  TFormAnalyseReserves = class(TFormMethodDialog)
...
var
  FormAnalyseReserves: TFormAnalyseReserves;
...
procedure TFormAnalyseReserves.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TFormAnalyseReserves.ToolButtonInputClick(Sender: TObject);
begin
  inherited;
...
procedure TFormAnalyseReserves.ListBoxInputNamesClick(Sender: TObject);
begin
  inherited;
  with dmBase do
  begin
...
procedure TFormAnalyseReserves.MenuItemParallelProfilesClick(Sender: TObject);
begin
...
procedure TFormAnalyseReserves.MenuItemVoronoiPolygonsClick(Sender: TObject);
begin
...
procedure TFormAnalyseReserves.CreateReportTable;
begin
  with dmBase do
  begin
    with TableOutput do
...
function TFormAnalyseReserves.GetDensity: double;
begin
  Result := 3;
  with dmBase do
    try
...
function TFormAnalyseReserves.GetMoisture: double;
begin
  Result := 0;
  with dmBase do
    try
...
procedure TFormAnalyseReserves.Sections;
...
procedure TFormAnalyseReserves.Cuttings(fldOreType: string);
    ...
    begin
      ...
      try
        ...
        if MinD = MinX then
          ...
          Result := usGlobals.fldY
        else if MinD = MinZ then
          Result := usGlobals.fldZ
        else
          Result := usGlobals.fldX;
      except
      end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ShowMessage('Real fields not found!');
        ModalResult := mrNone;
        TableInput.Close;
        Abort;
      end;
...
procedure TFormAnalyseReserves.CuttingsWithOreSort(fldOreType, fldOreSort: string);
...
procedure TFormAnalyseReserves.RegularBlocks;
...
procedure TFormAnalyseReserves.SobolevskyGrid;
begin
  RegularBlocks;
end;

//-------------------- Triangular Prizms ------------------------------\\
procedure TFormAnalyseReserves.TrianglePrizms;
var
  TableVertex: TTable;
  CurrentName: string;
  I, J: integer;
...
procedure TFormAnalyseReserves.VoronoiPolygons;
var
...
procedure TFormAnalyseReserves.ButtonOKClick(Sender: TObject);
begin
  inherited;
  with dmBase do
