...
type
  TfrmTableWindow = class(TFormFirst)
...
var
  frmTableWindow: TfrmTableWindow;
...
procedure TfrmTableWindow.FormCreate(Sender: TObject);
var
  S: String;
begin
  inherited;
...
procedure TfrmTableWindow.OpenTable(ATableName: TFileName);
begin
...
procedure TfrmTableWindow.DBGridMasterDblClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      FormDrawFillStyle := TFormDrawFillStyle.Create(Self);
      try
        FormDrawFillStyle.Caption := DBGridMaster.SelectedField.FieldName;
          // Self.Table.Edit;
        if FormDrawFillStyle.ShowModal = MrOk then
          // Self.Table.Post
        else
          // Self.Table.Cancel;
        finally
          FreeAndNil(FormDrawFillStyle);
        end;
      end
    else
    begin
...
//----------------------------------------------------------------------------
procedure TfrmTableWindow.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmTableWindow.EditTableStatisticExecute(Sender: TObject);
begin
...
procedure TfrmTableWindow.EditFieldInsertExecute(Sender: TObject);
begin
...
procedure TfrmTableWindow.EditFieldDeleteExecute(Sender: TObject);
var
  I: Integer;
  Separator: Char;
begin
...
procedure TfrmTableWindow.EditFieldCalculateExecute(Sender: TObject);
var
  I: Integer;
begin
...
procedure TfrmTableWindow.DelAllRecords;
begin
...
procedure TfrmTableWindow.EditFieldRenameExecute(Sender: TObject);
var
  I: Integer;
...
procedure TfrmTableWindow.EditQueryExecute(Sender: TObject);
var
  I: Integer;
  Comma: string[1];
begin
...
procedure TfrmTableWindow.EditTableClearExecute(Sender: TObject);
begin
...
procedure TfrmTableWindow.EditTableAddExecute(Sender: TObject);
var
...
procedure TfrmTableWindow.FormActivate(Sender: TObject);
begin
  FormGeoblock.EditCopy.Enabled := True;
  FormGeoblock.EditPaste.Enabled := True;
  DragAcceptFiles(Handle, True);
end;

procedure TfrmTableWindow.FormDeactivate(Sender: TObject);
begin
  FormGeoblock.EditCopy.Enabled := False;
  FormGeoblock.EditPaste.Enabled := False;
end;

procedure TfrmTableWindow.CopyToClipboard(Sender: TObject);
var
  I, J: Integer;
  Str: string;
...
procedure TfrmTableWindow.PasteFromClipboard(Sender: TObject);
...
procedure TfrmTableWindow.TableMasterAfterScroll(DataSet: TDataSet);
begin
  StaticText.Caption := IntToStr(DataSet.RecNo) + ' : ' +
    IntToStr(DataSet.RecordCount);
end;

procedure TfrmTableWindow.EditFieldLookupExecute(Sender: TObject);
begin
...
procedure TfrmTableWindow.DBGridMasterTitleClick(Column: TColumn);
var
  I, OldRecNo: Longint;
  MaxWidth: Longint;
  TextSize: TSize;
...
procedure TfrmTableWindow.DBGridMasterDrawColumnCell(Sender: TObject;
...
procedure TfrmTableWindow.DBGridMasterKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    VK_DELETE:
...
procedure TfrmTableWindow.DBLookupComboBoxKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    VK_RETURN:
...
procedure TfrmTableWindow.FormShow(Sender: TObject);
begin
  ToolBar.Width := ControlBar.Width - DBNavigator.Width - 10;
end;

procedure TfrmTableWindow.SelectAll(Sender: TObject);
var
  I: Integer;
begin
  TableMaster.DisableControls;
...
procedure TfrmTableWindow.DBGridMasterMouseDown(Sender: TObject;
...
procedure TfrmTableWindow.EditTableUpdateStructureExecute(Sender: TObject);
var
  I: Integer;
  Comma: string[1];
  Query: TQuery;
...
procedure TfrmTableWindow.ToolButtonEditFieldCalculateDragOver(Sender,
  Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  Accept := Source is TCustomEdit;
end;

procedure TfrmTableWindow.ReadIniFile;
begin
...
procedure TfrmTableWindow.WriteIniFile;
begin
  IniFile := TIniFile.Create(ChangeFileExt(ParamStr(0), '.ini'));
  with IniFile do
    try
...
procedure TfrmTableWindow.ToolButtonEditFieldCalculateDragDrop(Sender,
...
procedure TfrmTableWindow.SaveAs(ATableName: TFileName);
var
  NewName: string;
begin
  NewName := ATableName;
...
procedure TfrmTableWindow.EditTableFindExecute(Sender: TObject);
var
  RecNo: Integer;
begin
...
procedure TfrmTableWindow.TableMasterBeforeClose(DataSet: TDataSet);
begin
  if DataSet.State = DsEdit then
    DataSet.Post;
end;

procedure TfrmTableWindow.EditCopyFieldNamesExecute(Sender: TObject);
var
  I: Integer;
  Str: string;
begin
...
procedure TfrmTableWindow.SpeedButtonAllowDeleteRecordsClick(Sender: TObject);
begin
